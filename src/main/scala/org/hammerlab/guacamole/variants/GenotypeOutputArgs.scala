...
trait GenotypeOutputArgs extends Args with ParquetArgs {
   ...
   */
  def writeVariantsFromArguments(genotypes: RDD[BDGGenotype]): Unit = {
    val subsetGenotypes =
      if (maxGenotypes > 0) {
    ...
    val outputPath = variantOutput.stripMargin
    ...
    if (outputPath.isEmpty || outputPath.toLowerCase.endsWith(".json")) {
      ...
      val coalescedSubsetGenotypes =
        if (outChunks > 0)
          subsetGenotypes.coalesce(outChunks)
        else
          subsetGenotypes
