...
class SomaticStandardCallerSuite extends GuacFunSuite with TableDrivenPropertyChecks {
  ...
  test("no indels") {
    ...
    val normalPileup = Pileup(normalReads, "chr1", 2, contigSequence = simpleReference.getContig("chr1"))
    ...
    val tumorPileup = Pileup(tumorReads, "chr1", 2, contigSequence = simpleReference.getContig("chr1"))
  ...
  test("multiple-base deletion") {
    ...
    val normalPileup = Pileup(normalReads, "chr4", 4, contigSequence = simpleReference.getContig("chr4"))
    ...
    val tumorPileup = Pileup(tumorReads, "chr4", 4, contigSequence = simpleReference.getContig("chr4"))
  ...
  test("single-base insertion") {
    ...
    val tumorPileup = Pileup(tumorReads, "chr1", 3, contigSequence = simpleReference.getContig("chr1"))
  ...
  test("multiple-base insertion") {
    ...
    val alleles = SomaticStandard.Caller.findPotentialVariantAtLocus(
      Pileup(tumorReads, "chr1", 3, contigSequence = simpleReference.getContig("chr1")),
      Pileup(normalReads, "chr1", 3, contigSequence = simpleReference.getContig("chr1")),
      oddsThreshold = 2
    )
    alleles.size should be(1)
  ...
  test("insertions and deletions") {
    ...
    def testLocus(referenceName: String, locus: Int, refBases: String, altBases: String) = {
      val alleles = SomaticStandard.Caller.findPotentialVariantAtLocus(
        Pileup(tumorReads, referenceName, locus, contigSequence = simpleReference.getContig("chr3")),
        Pileup(normalReads, referenceName, locus, contigSequence = simpleReference.getContig("chr3")),
        oddsThreshold = 2
      )
      alleles.size should be(1)
