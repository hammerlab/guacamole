...
class VariantSupportSuite extends GuacFunSuite with TableDrivenPropertyChecks {
      ...
      withClue(s"$contig:$locus") {
        ...
        val pileup =
          Pileup(
            ...
            contig,
            locus,
            reference = grch37Reference.getContig(contig)
          )
  ...
  test("read evidence for mid-deletion") {
    val pileup = Pileup(gatkReads("20:10006822-10006823"), "20", 10006822, reference = grch37Reference.getContig("20"))
    assertAlleleCounts(pileup, ("C", "", 6), ("C", "C", 2))
  }

  test("read evidence for simple snvs 2") {
    val reads = gatkReads("20:10000624-10000625")
    val pileup = Pileup(reads, "20", 10000624, reference = grch37Reference.getContig("20"))
